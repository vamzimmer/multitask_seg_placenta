...
"""
    Multi-task model for classification (with attention layers [1]) and segmentation
...
class MyLinearAttentionBlock(nn.Module):
    ...
    def forward(self, l, g):
        N, C, W, H, D = l.size()
        c = self.compatibility_score(l + g)  # batch_sizex1xWxH
...
class UNet3D(nn.Module):
    def __init__(self, inchannels, n_classes, outchannels=2, first_channels=32, image_size=(256, 256),
                 levels=5, dropout=False, dropout_rate=0.2, concatenation=False,
                 attention_layers_pos=(1, 2), task='segment'):
        ...
        for i in range(self.levels - 1, 1, -1):
            block = nn.Sequential(
                ...
                nn.Upsample(scale_factor=2),
                # main difference to my previous unet architecture:
    ...
    def forward(self, x):
        ...
        for i in range(self.levels):
            ...
            if self.task == 'classify' or self.task == 'multitask':
                l_all.append(l_i)
...
class ResUNet3D(nn.Module):
    def __init__(self, inchannels, n_classes, outchannels=2, first_channels=32, image_size=(256, 256),
        ...
        self.concatenation = concatenation
        concat_factor = 1 if not self.concatenation else 2
        # print("Concat factor: {}".format(concat_factor))
        self.attention_layers_pos = attention_layers_pos
        self.task = task
        ...
        #   Attention layers
        #
        if len(self.attention_layers_pos) > 0:
            self.attention_layers = nn.ModuleList(
                [MyLinearAttentionBlock(in_features=self.n_channels[-1], normalize_attn=True) for i in
                 self.attention_layers_pos])
        ...
        self.dense = nn.Sequential(
            nn.Conv3d(self.n_channels[-1], self.n_channels[-1], kernel_size=self.layer_sizes[-1], stride=1, padding=0, bias=True),
            # nn.GroupNorm(groups, outChannel),
            nn.LayerNorm([self.n_channels[-1], 1, 1, 1]),
            # nn.LayerNorm([1,self.n_channels[-1], 1, 1, 1]),
            nn.ReLU(inplace=True)
        )
        if len(self.attention_layers_pos) > 0:
            self.classify = LinearBlock(self.n_channels[-1] * len(self.attention_layers_pos), self.N_CLASSES)
        else:
            self.classify = LinearBlock(self.n_channels[-1], self.N_CLASSES)
    ...
    def forward(self, x):
        ...
        if self.task == 'segment' or self.task == 'multitask':
            x = l_i
            for i in range(self.levels):
